...
class QR {
 ...
 private:
  ...
  void factorize() {
    ...
    EIGEN_CUSOLVER_CHECK(cusolverDnXgeqrf_bufferSize(ctx_.cusolver_, ctx_.params_.p, m_, n_, dtype, d_qr_.get(), lda_,
                                                     dtype, d_tau_.get(), dtype, &dev_ws, &host_ws));

    ctx_.ensure_scratch(dev_ws);
    ctx_.h_workspace_.resize(host_ws);

    EIGEN_CUSOLVER_CHECK(cusolverDnXgeqrf(ctx_.cusolver_, ctx_.params_.p, m_, n_, dtype, d_qr_.get(), lda_, dtype,
