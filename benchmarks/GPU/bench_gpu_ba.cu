...
static NormalEquations build_normal_equations(const BALProblem& problem, double lambda = 1.0) {
  ...
  fprintf(stderr, "Normal equations: H is %dx%d, nnz=%ld, |r|=%.6e\n", (int)H.rows(), (int)H.cols(),
          (long)H.nonZeros(), rnorm);

  return {std::move(H), std::move(g), std::move(residual), rnorm, num_residuals, num_params,
          (long)J.nonZeros()};
}
...
int main(int argc, char** argv) {
  ...
  if (bal_file) {
    ...
    fprintf(stderr,
            ...
            "|residual|:    %.6e\n"
            "==========================================\n\n",
            bal_file, g_problem.num_cameras, g_problem.num_points, g_problem.num_observations,
            g_neq.jacobian_rows, g_neq.jacobian_cols, g_neq.jacobian_nnz, (int)g_neq.H.rows(),
            (int)g_neq.H.cols(), (long)g_neq.H.nonZeros(), g_neq.residual_norm);
  }
